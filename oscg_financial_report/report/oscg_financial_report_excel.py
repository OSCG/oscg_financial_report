...
class Parser(report_sxw.rml_parse,common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update( {
            ...
            'get_filter_date': self.get_filter_date,
            'get_filter_period': self.get_filter_period,
            'get_start_date':self._get_start_date,
            'get_end_date':self._get_end_date,
            'get_comparison':self.get_comparison,
            'get_comparison_value':self.get_comparison_value,
            'get_credit_debit':self.get_credit_debit,
        })
    ...
    def get_filter_date(self, data):
        if data['filter'] == 'filter_date':
            return True
        else:
            return False
            
    def get_filter_period(self, data):
        if data['filter'] == 'filter_period':
            return True
        else:
            return False
            
    def get_credit_debit(self, data):
        if data['debit_credit']:
            return True
        else:
            return False
    
    def get_comparison_value(self, data):
            ...
            if account_ids:
                for account in account_obj.browse(self.cr, self.uid, account_ids, context=new_context):
                    ...
                    if account.parent_id:
                        parent_list.append(account.parent_id)
                    for parent in parent_list:
                        blank += '   '
                        if parent.parent_id:
                            parent_list.append(parent.parent_id)
                    vals = {
                        'name': blank + account.code + ' ' + account.name,
...
#report_sxw.report_sxw('report.twt.financial.report', 'account.financial.report',
#    'addons/oscg_report_twt/report/kaertech_financial_report_excel.ods', parser=kaertech_financial_report, header='internal')
